...
import { process_phone_no, processTimeStamp, processString } from "../helper/preprocess_data.helper.js";
...
export default class doctorController {
    ...
    createDoctorByName = asyncHandler(async (req, res, next) => {
        ...
        if (doctor?.rows[0]?.length == 0) throw new apiError(500, "Some error occured during doctor creation.");
    ...
    updateDoctor = asyncHandler(async (req, res, next) => {
        ...
        if (!id && !phone) {
            throw new apiError(400, "Provide either 'id' or 'phone' to identify the doctor for update.");
        }
        ...
        const addField = (value, dbColumn) => {
            if (value !== undefined && value !== null) {
                ...
                queryParams.push(['first_name', 'last_name', 'status'].includes(dbColumn) ? processString(value) : value);
            }
        };
        ...
        if (location_locality !== undefined || req.body.latitude !== undefined || req.body.longitude !== undefined) {
        ...
        res.status(200).json(new apiResponse(200, updatedResult.rows[0], "Doctor successfully updated"));
    });
    ...
    deleteDoctor = asyncHandler(async (req, res, next) => {
        ...
        if (!id && !phone) {
            throw new apiError(400, "Provide id or phone number of the doctor to delete");
        }
    ...
    // BATCH: Create Online Doctors (Existing)
